...
This skill focuses only on initialization — not decomposition. After initialization, run `node-decompose` to decompose the pending root node.
...
- Run `node-decompose` to begin decomposing the pending root node
```
